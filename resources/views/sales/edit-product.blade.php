    ...
    <div>
        <section class="bg-white :bg-gray-900">
            <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                <h2 class="mb-4 text-xl font-bold text-gray-900 :text-white">Edit Sales product</h2>
                <form action="{{ route('updateSales', ['idSales' => $sales->id]) }}" method="POST">
                    @csrf
                    <div>
                        <label for="price"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        <div>
                            <label for="qty"
                                class="block mb-2 text-sm font-medium text-gray-900 :text-white">Quantity</label>
                            <input type="number" value="{{ $sales->salesDetails->qty }}" oninput="calculatePrice()"
                                min="0" max="{{ $maxStock }}" name="qty" id="qty"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 :bg-gray-700 :border-gray-600 :placeholder-gray-400 :text-white :focus:ring-primary-500 :focus:border-primary-500">
                        </div>
                        <div class="w-full">
                            <label for="price_sale" class="block mb-2 text-sm font-medium text-gray-900 :text-white">Price
                                Sale</label>
                            <input disabled type="text" onchange="calculatePrice()"
                                value="Rp. {{ number_format($sales->salesDetails->price_sale, 0, ',', '.') }}"
                                name="price_sale" id="price_sale"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 :bg-gray-700 :border-gray-600 :placeholder-gray-400 :text-white :focus:ring-primary-500 :focus:border-primary-500">
                            <input type="hidden" name="hiddenPriceSale" id="hiddenPriceSale" value="{{$sales->salesDetails->price_sale}}">
                        </div>
                        <div class="w-full">
                            <label for="price"
                                class="block mb-2 text-sm font-medium text-gray-900 :text-white">Price</label>
                            <input disabled type="text" onchange="calculatePrice()"
                                value="Rp. {{ number_format($sales->salesDetails->price, 0, ',', '.') }}" name="price"
                                id="price"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 :bg-gray-700 :border-gray-600 :placeholder-gray-400 :text-white :focus:ring-primary-500 :focus:border-primary-500">
                            <input type="hidden" name="hiddenPrice" id="hiddenPrice">
                        </div>
                        <div class="w-full">
                            <label for="status" class="block mb-2 text-sm font-medium text-gray-900 :text-white">Choose a
                                status:</label>
                            <select name="status" id="status"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 :bg-gray-700 :border-gray-600 :placeholder-gray-400 :text-white :focus:ring-primary-500 :focus:border-primary-500">
                                <option value="wait_for_proccess"
                                    {{ $sales->salesDetails->status == 'wait_for_process' ? 'selected' : '' }}>Wait for
                                    Process</option>
                                <option value="ongoing" {{ $sales->salesDetails->status == 'ongoing' ? 'ongoing' : '' }}>
                                    Ongoing</option>
                                <option value="complete" {{ $sales->salesDetails->status == 'complete' ? 'complete' : '' }}>
                                    Complete</option>
                                <option value="return" {{ $sales->salesDetails->status == 'return' ? 'return' : '' }}>
                                    Return</option>
                            </select>
                        </div>
                    </div>
